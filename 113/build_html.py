...
function isCorrectAnswer(q, chosen) {{
  if (isMultiple(q)) return q.answer.includes(chosen);
...
// For multi-answer: chosen = the number clicked, correct = first answer
...
const answered = new Map(
  Object.entries(_savedAnswered).map(([k, chosen]) => {{
    ...
    const correct = q ? (isMultiple(q) ? q.answer[0] : q.answer) : chosen;
    return [+k, {{ chosen: +chosen, correct }}];
  }})
);
...
function renderCard(q) {{
  ...
  if (ans) {{
    if (multi) {{
      ...
      const hitCorrect = q.answer.includes(ans.chosen);
  ...
  const numBadge = q.isNumeric && !multi ? ' 🔢' : '';
...
document.getElementById('main-grid').addEventListener('click', e => {{
  ...
  if (optBtn && !optBtn.disabled) {{
    ...
    answered.set(id, {{ chosen, correct: isMultiple(q) ? q.answer[0] : q.answer }});
    saveAnswered();
    updateProgress();

    // For wrong tracking: wrong if chosen is not in correct answers
    if (isCorrectAnswer(q, chosen)) unmarkWrong(id);
    else markWrong(id);
  ...
  reader.onload = ev => {{
    try {{
      ...
      for (const [k, chosen] of Object.entries(data.answered)) {{
        const id = +k;
        const q = QUESTIONS.find(q => q.id === id);
        if (q) answered.set(id, {{chosen: +chosen, correct: isMultiple(q) ? q.answer[0] : q.answer}});
      }}
      saveAnswered();
